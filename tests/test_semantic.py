...
@pytest.mark.skipif(not sci_installed, reason="iscc-sci not installed")
def test_code_iscc_text_keep_experimental(pdf_file):
    result = idk.code_iscc(pdf_file, text_keep=True, experimental=True)
    assert result.text.startswith("Bitcoin: A Peer-to-Peer Electronic Cash System")


@pytest.mark.skipif(not sci_installed, reason="iscc-sci not installed")
def test_code_semantic_image_raises(jpg_file):
    with patch("iscc_sdk.is_installed", return_value=False):
        with pytest.raises(idk.EnvironmentError):
            idk.code_image_semantic(jpg_file)


@pytest.mark.skipif(not sct_installed, reason="iscc-sct not installed")
def test_code_semantic_text_raises(pdf_file):
    with patch("iscc_sdk.is_installed", return_value=False):
        with pytest.raises(idk.EnvironmentError):
