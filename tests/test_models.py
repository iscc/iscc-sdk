...
def test_IsccMeta_json():
    im = IsccMeta(iscc="ISCC:MEAJU5AXCPOIOYFL")
    assert im.json() == (
        '{"@context": "http://purl.org/iscc/context/0.3.5.jsonld", "@type": '
        '"CreativeWork", "$schema": "http://purl.org/iscc/schema/0.3.5.json", "iscc": '
...
def test_IsccMeta_json_ld():
    im = IsccMeta(iscc="ISCC:MEAJU5AXCPOIOYFL")
    assert im.json_ld() == (
        b'{"$schema":"http://purl.org/iscc/schema/0.3.5.json","@context":"http://purl.'
        b'org/iscc/context/0.3.5.jsonld","@type":"CreativeWork","iscc":"ISCC:MEAJU5AXC'
        b'POIOYFL"}'
    )
